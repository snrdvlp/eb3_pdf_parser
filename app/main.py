...
async def extract_json_endpoint(
    ...
    sims = db.search_similar_pdf(category.lower(), pdf_bytes, top_k=3)   # Get 3 for few-shot!
    if not sims:
        return JSONResponse(
            status_code=400,
    ...
    matched_json = sims[0]['json_data']
    matched_carrier = matched_json.get('Carrier Name', '')
    matched_plan = matched_json.get('Plan Name', '')
